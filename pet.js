...
(function() {
    ...
    function setExpression(type) {
        [imgNormal, imgWink, imgChibi].forEach(img => {
            if (img) img.style.display = 'none';
        });
        
        if (type === 'wink' && imgWink) imgWink.style.display = 'inline';
        else if (type === 'chibi' && imgChibi) imgChibi.style.display = 'inline';
        else if (imgNormal) imgNormal.style.display = 'inline';
    }
    ...
    shipper.onmouseover = () => { 
        setExpression('wink'); 
        if (imgWink) imgWink.style.transform = 'scale(1.05)'; 
    };
    
    shipper.onmouseout = () => { 
        setExpression('normal'); 
        if (imgWink) imgWink.style.transform = 'scale(1)'; 
    };

    // 點擊事件 (修正關鍵點：增加對 secret-content 的存在檢查)
    shipper.onclick = () => {
        const secretElem = document.getElementById('secret-content');
        // 如果找不到 secret-content 標籤，就預設為 false，不會噴錯
        const isUnlocked = secretElem ? (secretElem.style.display === 'block') : false;
